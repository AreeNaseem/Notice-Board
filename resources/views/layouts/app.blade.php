...
<nav class="w-full py-4 border-t border-b bg-gray-100" x-data="{ open: false }">
    ...
    <div x-data="{ open: false }" class="w-full flex-grow sm:flex sm:items-center sm:w-auto">
        ...
        <div class="flex items-center">
               <!-- Home Link -->
                ...
                <div @click.away="open = false" class="relative">
                    <button @click="open = !open" class="hover:bg-blue-600 hover:text-white rounded py-2 px-4 mx-2">
                        CATEGORY
                    </button>
                    <div x-show="open" class="absolute mt-2 bg-white shadow-lg rounded z-50">
                        @foreach($categories as $category)
                            <a href="{{route('by-category', $category)}}"
                            class="block hover:bg-blue-600 hover:text-white rounded py-2 px-4 mx-2 whitespace-no-wrap">
                                {{$category->title}}
                            </a>
                        @endforeach
                    </div>
                    ...
                    <!-- Contact Link -->
                    <a href="{{route('about-us')}}" class="hover:bg-blue-600 hover:text-white rounded py-2 px-4 mx-2">Contact</a>
                </div>
